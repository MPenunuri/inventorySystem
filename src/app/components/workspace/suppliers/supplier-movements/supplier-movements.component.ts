...
@Component({
  selector: 'app-supplier-movements',
  standalone: true,
  imports: [
    CommonModule,
    LoadingComponent,
  ...
  templateUrl: './supplier-movements.component.html',
  styleUrl: './supplier-movements.component.scss',
})
export class SupplierMovementsComponent {
  supplierId?: number;
  supplierName?: string;
  ...
  ngOnInit(): void {
    const paramId = this.route.snapshot.paramMap.get('supplierId');
    const paramName = this.route.snapshot.paramMap.get('supplierName');
    if (paramId !== null) {
      this.supplierId = parseInt(paramId);
      this.setMovements();
    }
    if (paramName !== null) {
      this.supplierName = paramName;
    }
  }

  setMovements() {
    if (this.supplierId) {
      this.service.getSupplierRelatedMovements(this.supplierId).subscribe({
        next: (data) => {
          this.movements = data;
