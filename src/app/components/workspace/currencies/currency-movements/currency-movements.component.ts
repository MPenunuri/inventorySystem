...
@Component({
  selector: 'app-currency-movements',
  standalone: true,
  imports: [
    CommonModule,
    LoadingComponent,
  ...
  templateUrl: './currency-movements.component.html',
  styleUrl: './currency-movements.component.scss',
})
export class CurrencyMovementsComponent {
  currencyId?: number;
  currencyName?: string;
  ...
  setMovements() {
    if (this.currencyId) {
      this.service.getCurrencyRelatedMovements(this.currencyId).subscribe({
        next: (data) => {
          this.movements = data;
